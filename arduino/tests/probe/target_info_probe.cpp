#ifndef __STC_MCS251__
#error "the synthetic STC MCS251 frontend profile is not active"
#endif
...
#error "plain char must be unsigned for the candidate STC data model"
#endif

#ifdef __MSP430__
#error "the synthetic STC profile must not impersonate an MSP430 source target"
#endif

#ifdef MSP430
#error "the synthetic STC profile must not expose the legacy MSP430 macro"
#endif

#ifdef __SDCC_mcs251
#error "Clang must not impersonate SDCC"
#endif

static_assert(__CHAR_BIT__ == 8, "CHAR_BIT drift");
static_assert(__BYTE_ORDER__ == __ORDER_BIG_ENDIAN__, "endianness drift");
...
static_assert(sizeof(__SIZE_TYPE__) == 4 && alignof(__SIZE_TYPE__) == 1,
              "size_t model drift");
...
static_assert(sizeof(void *) == 3 && alignof(void *) == 1,
              "data pointer layout drift");
static_assert(sizeof(FunctionPointer) == 3 && alignof(FunctionPointer) == 1,
              "function pointer layout drift");
static_assert(sizeof(FunctionReferenceHolder) == 4 &&
...
// The locked STC Itanium profile encodes both fields with the 24-bit MCS251
// program-pointer width, independently of the 32-bit ptrdiff_t type.
static_assert(sizeof(DataMember) == 3 && alignof(DataMember) == 1,
              "Itanium data-member pointer shape changed");
static_assert(sizeof(MethodMember) == 6 && alignof(MethodMember) == 1,
              "Itanium method-member pointer shape changed");
...
extern "C" long bridge_scalars(int narrow, long wide, void *pointer, float f,
                               double d, long double ld) {
  return wide + narrow + (pointer != nullptr) + (f > 0) + (d > 0) + (ld > 0);
}
...
using AddressSpace1Int = int __attribute__((address_space(1)));

extern "C" AddressSpace1Int *
bridge_explicit_address_space(AddressSpace1Int *pointer) {
  return pointer;
}
