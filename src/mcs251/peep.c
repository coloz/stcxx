...
/*-----------------------------------------------------------------*/
/* mcs251DeadMove - dispatch condition deadmove between            */
/* - remove pop/push                                               */
/* - remove mov r%1,%2                                             */
/*-----------------------------------------------------------------*/
bool
mcs251DeadMove (const char *reg, lineNode *currPl, lineNode *head)
{
  dbglog_deadmove (printf ("mcs251DeadMove  reg: %s  line: %s\n", reg, currPl->line));
...
/*-----------------------------------------------------------------*/
/* mcs251notUsed - Check that 'what' is never read after 'endPl'.  */
/*-----------------------------------------------------------------*/
bool
mcs251notUsed (const char *what, lineNode *endPl, lineNode *head)
{
  dbglog_deadmove (printf ("mcs251notUsed %s  after line: %s\n", what, endPl->line));
  ...
  if (!strcmp (what, "acc"))
    return (mcs251notUsed ("a", endPl, head));
...
/*----------------------------------------------------------------------------*/
/* mcs251notUsedFrom - Check that 'what' is never read from 'label'. */
/*----------------------------------------------------------------------------*/
bool
mcs251notUsedFrom (const char *what, const char *label, lineNode *head)
{
  for (lineNode *cpl = head; cpl; cpl = cpl->next)
    if (cpl->isLabel && !strncmp (label, cpl->line, strlen(label)))
      return (mcs251notUsed (what, cpl, head));
...
bool
mcs251CanAssign (const char *dst, const char *src, const char *exotic)
{
