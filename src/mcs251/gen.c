/*-------------------------------------------------------------------------
  gen.c - source file for MCS251 code generation
...
/* ── Lower layers: emission, ASMOP, value access ──
 * Included from gen_lower.c.inc to keep this file at a manageable size.
 * This is the same translation unit; all static symbols are shared.
 */
...
saveRegisters (iCode * lic)
{
  ...
  if (_G.currentFunc && IFFUNC_CALLEESAVES (_G.currentFunc->type))
    {
      ...
      rsave = bitVectCopy (mcs251_allBankregs ());
      if (!inExcludeList ("bits"))
        {
          rsave = bitVectUnion (rsave, mcs251_allBitregs ());
          BitBankUsed = 1;
        }
    }
  else
  ...
  if (options.useXstack)
    {
      bitVect *rsavebits = bitVectIntersect (bitVectCopy (mcs251_allBitregs ()), rsave);
      ...
      if (count == 1)
      ...
      else if (count != 0)
        {
          if ((FUNC_REGBANK (currFunc->type) == 0) &&
            ...
            {
              bool resultInR0 = bitVectBitValue (mcs251_rUmaskForOp (IC_RESULT (ic)), R0_IDX);
              if (resultInR0)
                {
                  emitpush (REG_WITH_INDEX (R0_IDX)->dname);
                }
      ...
      while (offset < size)
        {
          ...
          emitcode ("mov", "%s,%s", mcs251_fReturn[offset++], l);
        }
      freeAsmop (oper, NULL, ic, TRUE);
      offset = 0;
      while (offset < size)
        {
          emitpush (mcs251_fReturn[offset++]);
        }
      return;
    }
  ...
  for (i = 0; i < 8; i++)
    {
      if (options.useXstack)
        {
          emitcode ("mov", "a,(%s+%d)", mcs251_regs[i].base, 8 * bank + mcs251_regs[i].offset);
          emitcode ("movx", "@%s,a", r->name);
          _G.stack.xpushed++;
          if (--count)
            emitcode ("inc", "%s", r->name);
        ...
        {
          char buf[16] = "";
          SNPRINTF (buf, 16, "(%s+%d)", mcs251_regs[i].base, 8 * bank + mcs251_regs[i].offset);
          emitpush (buf);
        }
    }
      ...
      if (options.useXstack)
        {
          emitcode ("dec", "%s", r->name);
          emitcode ("movx", "a,@%s", r->name);
          emitcode ("mov", "(%s+%d),a", mcs251_regs[i].base, 8 * bank + mcs251_regs[i].offset);
          _G.stack.xpushed--;
        }
      else
        {
...
pushbigreturn (operand *result)
{
  ...
  if (sym->onStack && !options.useXstack)
    {
...
genCall (iCode * ic)
{
  ...
  if (IS_LITERAL (etype))
    {
      emitcode (call, "0x%06X", ulFromVal (OP_VALUE (IC_LEFT (ic))));
    }
  else
    {
...
genPcall (iCode * ic)
{
  ...
  if (IS_LITERAL (etype))
    ...
    {
      if (_G.sendSet)      /* the send set is not empty */
        {
      ...
      for (i = 0; i < MCS251_BANK_REG_COUNT; i++)
        {
          if (mcs251_regs[i].type != REG_BIT)
            {
              if (EQ (mcs251_regs[i].base, "0"))
                emitcode ("", "%s !equ !constbyte", mcs251_regs[i].dname, 8 * rbank + mcs251_regs[i].offset);
              else
                emitcode ("", "%s !equ %s + !constbyte", mcs251_regs[i].dname, mcs251_regs[i].base, 8 * rbank + mcs251_regs[i].offset);
            }
        }
    }
  ...
  if (IFFUNC_ISISR (ftype))
    {
      ...
      rsavebits = bitVectIntersect (bitVectCopy (mcs251_allBitregs ()), sym->regsUsed);
...
//                      werror (W_FUNCPTR_IN_USING_ISR);
  ...
  while (ric && ric->argreg > 1000 && ric->next && ric->next->op == RECEIVE)
    ric = ric->next;
  bool accIsFree = !(mcs251IsParmInCall (ftype, "a"));
  if (ric && ric->argreg <= 1000 && ric->result)
    {
          ...
          for (ofs = 0; ofs < getSize (rsym->type); ofs++)
            {
              ...
              emitpush (mcs251_fReturn[logicalOffset]);
      ...
      /* to free up the accumulator. */
      else if (rsym && rsym->nRegs && getSize (rsym->type) == 4) // Should be >= 4, but the loop below is fragile. TODO: Use genMove here, enable for >= 4.
        {
          ...
          for (ofs = 0; ofs < getSize (rsym->type); ofs++)
            {
              emitcode ("mov", "%s,%s", rsym->regs[ofs]->name, mcs251_fReturn[ofs]);
            }
          genLine.lineElement.ic = ic;
          ric->generated = 1;
          accIsFree = true;
...
genEndFunction (iCode * ic)
{
          ...
          if (sym->regsUsed)
            {
              ...
              for (i = sym->regsUsed->size; i >= 0; i--)
                {
                  if (bitVectBitValue (sym->regsUsed, i) || (mcs251_ptrRegReq && (i == R0_IDX || i == R1_IDX)))
                    emitpop (REG_WITH_INDEX (i)->dname);
                }
            }
          else if (mcs251_ptrRegReq)
            {
  ...
  regsUsed = newBitVect (mcs251_nRegs);
  lnp = lineBodyEnd;
  while (lnp)
    {
      ...
      if (lnp->ic && lnp->ic->op == FUNCTION &&
          ...
          !bitVectBitsInCommon (mcs251_allBankregs (), regsUsed) &&
  ...
  regsUnneeded = newBitVect (mcs251_nRegs);
      ...
      if (AOP_TYPE (IC_LEFT (ic)) == AOP_DPTR)
        {
          ...
          if (mcs251_getTempRegs (tempRegs, tempRegCount, ic))
            {emitcode(";", "A");
              emitcode ("mov", "a,%s", bp);
  ...
  if (IS_BIT (_G.currentFunc->etype))
  ...
  else
    {
      ...
      while (pushed)
        {
          pushed--;
          if (!EQ (mcs251_fReturn[pushed], "a"))
            emitpop (mcs251_fReturn[pushed]);
          else
            emitpop ("acc");
        }
    }
...
genLabel (iCode * ic)
{
  ...
  _G.dptrCache.valid = false;  /* control-flow join invalidates dptr cache */
...
genPlusIncr (iCode * ic)
{
  ...
  /* we can if the aops of the left & result match or
     ...
     same */
  if (sameRegs (AOP (IC_LEFT (ic)), AOP (IC_RESULT (ic))) &&
      AOP_TYPE (IC_LEFT (ic)) != AOP_MCS251_STK)
    {
      const char *l = opGet (IC_LEFT (ic), 0, FALSE, FALSE);
      bool mcs251_step_operand = EQ (l, "a") ||
        (l[0] == 'r' && l[1] >= '0' && l[1] <= '7' && l[2] == '\0');
...
mcs251DwordForNativeTuple (const asmop *aop)
{
  ...
  if (aop->type != AOP_REG || aop->size != 4)
    return NULL;
...
gencjneshort (operand * left, operand * right, symbol * lbl)
{
  ...
  if (IS_AOP_IMMEDIATE (right) && AOP_TYPE (left) != AOP_DIR &&
      AOP_TYPE (left) != AOP_SFR && !IS_AOP_IMMEDIATE (left))
    {
      while (size--)
        {
          char *l = Safe_strdup (opGet (left, offset, FALSE, FALSE));
...
mcs251WriteBackPostincrementedPointer (operand *pointer)
{
  ...
  wassert (AOP_TYPE (pointer) == AOP_DPTR);
...
genDataPointerSet (operand * right, operand * result, iCode * ic)
{
  ...
  /* Remove the destination's immediate marker before emitting each byte. */
  char *l = Safe_strdup (opGet (result, 0, FALSE, TRUE) + 1);
  for (offset = 0; offset < size; offset++)
    {
      struct dbuf_s dbuf;
      int logicalOffset = mcs251PointerByteOffset (
...
genFarPointerSet (operand * right, operand * result, iCode * ic, iCode * pi)
{
  ...
  /* so dptr now contains the address */
  aopOp (right, ic, FALSE);
  mcs251FarSource = AOP_TYPE (right) == AOP_DPTR;
  if (mcs251FarSource)
    {
...
genJumpTab (iCode * ic)
{
  ...
  if ((count <= 7) ||
      ...
      options.acall_ajmp)
    {
      /* MCS251 uses four-byte EJMP table entries. */
    ...
    {
      /* Use one table for each byte of the 24-bit destination. */
      jtablo = newiTempLabel (NULL);
      jtabhi = newiTempLabel (NULL);
      jtabext = newiTempLabel (NULL);

      /* Keep the selector in B while DPX addresses the component tables. */
      aopOp (cond, ic, FALSE);
      l = opGet (cond, 0, FALSE, FALSE);
      wassertl (!BINUSE, "B was in use");
      MOVB (l);
      ...
      /* Reconstruct DPX and jump through an otherwise unallocated dword
         register so ABI argument registers remain untouched. */
      MOVA (l);
  ...
  if (ic->argreg <= 1000)
    {
      ...
      if ((isOperandInFarSpace (IC_RESULT (ic)) ||
           isOperandInPagedSpace (IC_RESULT (ic))) && (OP_SYMBOL (IC_RESULT (ic))->isspilt || IS_TRUE_SYMOP (IC_RESULT (ic))))
        {
          ...
          if (!receivingA)
            {
              if (size == 1 || mcs251_getTempRegs (tempRegs, size - 1, ic))
                {
            ...
            {
              if (mcs251_getTempRegs (tempRegs, size, ic) && size <= 4 /* Workaround for issue: For size >= 4, mcs251_getTempRegs will give us regs in use for param, making us overwrite param bytes still needed */)
                {
                  for (offset = 0; offset < size; offset++)
          ...
          offset = mcs251_fReturnSize - size;
          while (size--)
            {
              emitpush ((!EQ (mcs251_fReturn[mcs251_fReturnSize - offset - 1], "a") ? mcs251_fReturn[mcs251_fReturnSize - offset - 1] : "acc"));
              offset++;
            }
          aopOp (IC_RESULT (ic), ic, FALSE);
          size = AOP_SIZE (IC_RESULT (ic));
...
/*-----------------------------------------------------------------*/
/* mcs251_genCode - generate code for 8051 based controllers            */
/*-----------------------------------------------------------------*/
void
mcs251_genCode (iCode * lic)
{
  iCode *ic;
  int cln = 0;
#ifdef _DEBUG
          ...
          for (i = 0; i < 8; i++)
            {
              if (bitVectBitValue (ic->rMask, i))
                {
                  int offset = mcs251_regs[i].offset;
...
bool
mcs251IsReturned (const char *what)
{
  if (!strcmp(what, "dptr"))
    return (mcs251IsReturned ("dpl") || mcs251IsReturned ("dph"));
...
bool
mcs251IsRegArg (struct sym_link *ftype, int i, const char *what)
{
  if (what && !strcmp(what, "dptr"))
    return (mcs251IsRegArg (ftype, i, "dpl") || mcs251IsRegArg (ftype, i, "dph"));
...
bool
mcs251IsParmInCall (sym_link *ftype, const char *what)
{
  const value *args;
  int i;

  for (i = 1, args = FUNC_ARGS (ftype); args; args = args->next, i++)
    if (!(IS_SPEC (args->type) && SPEC_NOUN (args->type) == V_BIT) && mcs251IsRegArg (ftype, i, what)) // TODO: Allow bits, once supported in aopArg!
      return true;
  return false;
}
